...
public interface WaterBillRepo extends JpaRepository<WaterBill,Integer> {
    ...
    //查询某一水表下所有账单
    @Query("select w from WaterBill w where w.waterMeter.id = :meterId")
    public List<WaterBill> findAllByWaterMeterId(@Param("meterId") Integer meterId);

    //查询某一水表下所有未付账单
    @Query("select w from WaterBill w where w.waterMeter.id = :meterId and w.is_paid = 0")
    public List<WaterBill> findUnpaidByWaterMeterId(@Param("meterId") Integer meterId);

    //查询某一水表下所有已付账单
    @Query("select w from WaterBill w where w.waterMeter.id = :meterId and w.is_paid = 1")
